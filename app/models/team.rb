...
# app/models/team.rb
# class to manage team data and calculations
class Team
  ...
  validates :name, :expected_scores, :total_scores, :levels_config, :players, presence: true
  ...
  def initialize(name, players, config)
    ...
    @total_scores = 0
    @expected_scores = 0
    setup_team(players) unless players.nil?
  end
